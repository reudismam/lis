...
export default function ListItemsPublicacoes(props){
    ...
    const ListPublicacoes = items.map(item => {
       return <div className="listaItens" key={item.key}>
                <p>
                    <input type="text" id={item.key} value={item.title} onChange={(e)=>{
                        props.setUpdate(e.target.value, item.key, item.title )
                        }}/>
                    ...
                    <textarea maxLength={300} id={item.key} value= {item.text} onChange={(e)=>{
